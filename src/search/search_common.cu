...
struct relation {
    std::vector<index_key_t> indexed;
    device_array_wrapper<index_key_t> d_indexed;

    std::vector<value_t> payload;
    device_array_wrapper<value_t> d_payload;

    void make_device_accessible() {
        d_indexed = create_device_array_from(indexed, device_table_allocator<idexed_key_t>());
        d_value = create_device_array_from(d_payload, device_table_allocator<value_t>());
    }
};
...
struct query_data {
    ...
    void create_relations() {
        ...
        const float selectivity = config.selectivity;
