...
template<class T>
__global__ void index_test(T var){
    var.lookup(0);
    printf("Hello World from GPU!\n");
}
...
__global__ void ij_full_kernel(const lineitem_table_plain_t* lineitem, const unsigned lineitem_size, const part_table_plain_t* part, IndexStructureType index_structure) {
#if 1
    const char* prefix = "PROMO";
...
no_op_index<indexed_t, payload_t, device_index_allocator, host_allocator> idx;
//cuda_hello<<<1,1>>>(idx);
index_test<<<1,1>>>(idx.device_index);
cudaDeviceSynchronize();
//return 0;
//}
#if 1
...
*/
    ...
    printf("lookup_cycles: %lu", lookup_cycles, "\t scan_cycles: %lu\n", scan_cycles);
    cudaDeviceReset();
